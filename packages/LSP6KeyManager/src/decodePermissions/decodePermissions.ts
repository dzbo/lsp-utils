...
import { LSP6PermissionName } from '@lukso/lsp-utils-constants';
    ...
    if (decodedPermissionsType === 'boolean') {
        ...
        for (let index = 0; index < zeroStrippedPermissions.length * 4; index++) {
            ...
            if (decodedPermissionsType === 'BytesLike') {
                ...
                if (bytesLikePermissions !== ZeroHash) {
                    const permissionName = Object.keys(PERMISSIONS).filter(
                        (permission) => LSP6Permissions[permission] === bytesLikePermissions,
                    )[0];
