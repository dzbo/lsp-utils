...
// lsp-utils
import { getIssuedAssets } from '@lukso/lsp12-utils';
import { DigitalAsset, DigitalAssetsCreators, Issuer } from '@lukso/lsp-utils-constants';
...
export async function authenticateDigitalAssetCreators(
    ...
    for (const creator of fetchedCreators) {
        ...
        if (isAddress(digitalAsset)) {
            fetchedIssuedAssets = await getIssuedAssets(creator.address.toString(), provider);
        } else if (isAddressable(digitalAsset)) {
            fetchedIssuedAssets = provider
                ? await getIssuedAssets(creator.address.toString(), provider)
                : await getIssuedAssets(
                      creator.address.toString(),
                      digitalAsset.runner as Provider,
                  );
        }
